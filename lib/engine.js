...
Engine.prototype.start = function (callback) {
  ...
  try {
    this.bullQueue = bullQueueModule.init(this._config);
    log.info('bull queue initialized');
  } catch (err) {
    log.error('failed to initialize bull queue:', err && err.message ? err.message : err);
  }
...
Engine.prototype._handleUncaughtException = function (err) {
  ...
  try { bullQueueModule.close(); } catch (e) {}
  process.exit(1);
};
