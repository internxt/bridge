...
Engine.prototype.start = function (callback) {
  ...
  const { QUEUE_USERNAME, QUEUE_PASSWORD, QUEUE_HOST } = this._config.queue;
  ...
  // console.log('require sergio', MongoDBStorageAdapter);
  setTimeout(() => {
    ...
    this.networkQueue.connectAndRetry().then(() => {
...
Engine.prototype._configureApp = function () {
  ...
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      log.info(`${req.method} ${req.originalUrl} [FINISHED] ${durationInMilliseconds.toLocaleString()} ms`);
    });

    res.on('close', () => {
      const durationInMilliseconds = getDurationInMilliseconds (start);
      log.info(`${req.method} ${req.originalUrl} [CLOSED] ${durationInMilliseconds.toLocaleString()} ms`);
    });
  ...
  const protocol = this._config.server.ssl &&
