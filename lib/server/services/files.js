...
async function removeFileAndEnqueueDeletionTask(storage, networkQueue, { bucketId, userEmail, idFile }) {
  ...
  const beforePointerIsRemoved = async (pointer, bucketEntry) => {
    await enqueueDeleteShardMessage(storage, networkQueue, pointer, bucketEntry);
  };
...
async function enqueueDeleteShardMessage(storage, networkQueue, pointer, bucketEntry) {
