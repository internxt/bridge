...
AuthenticateMiddlewareFactory._basic = async function ({ User }, req, res, next) {
  const creds = basicauth(req);

  try {
    const user = await User.lookup(creds.name, creds.pass);

    if (!user.activated) {
      return next(errors.ForbiddenError('User account has not been activated'));
    }
