 ...
 * @param {Function} next
 */
UsersRouter.prototype.createUser = async function (req, res, next) {
  const self = this;
  ...
  if (req.body.referralPartner) {
    ...
    User.create(req.body.email, req.body.password, async function (err, user) {
      if (err) {
        return next(err);
      }
 ...
 * @param {Function} next
 */
UsersRouter.prototype._dispatchAndCreatePubKey = async function (
  user,
  req,
  res,
  next
  ...
  PublicKey.create(user, req.body.pubkey, async function (err, pubkey) {
    if (err) {
 ...
 * @param {Function} next
 */
UsersRouter.prototype._createUserWithOpts = async function (req, res, next) {
  const self = this;
  const User = this.storage.models.User;
  ...
  try {
    const partner = await Partner.findOne({ name: req.body.referralPartner });
 ...
 * @param {Function} next
 */
UsersRouter.prototype.reactivateUser = async function (req, res, next) {
  const self = this;
  const User = this.storage.models.User;
...
UsersRouter.prototype.destroyUser = async function (req, res, next) {
  const self = this;
 ...
 * @param {Function} next
 */
UsersRouter.prototype.confirmDestroyUser = async function (req, res, next) {
  const User = this.storage.models.User;
  ...
  try {
    ...
    log.error(
      '[USERS/CONFIRM_DESTROY]: Error while deactivating user: %s',
      err.message
    );
...
UsersRouter.prototype.confirmDestroyUserStripe = async function (req, res, next) {
  ...
  try {
    ...
    return next(
      new errors.InternalError('Error ocurred while deactivating user account')
    );
  }
};
