...
FramesRouter.prototype.createFrame = async function (req, res, next) {
  const { Bucket, Frame } = this.storage.models;
  const { body } = req;
  ...
  if (!body.bucket) {
    ...
    return Frame.create(req.user, function (err, frame) {
      if (err) {
  ...
  Bucket.findOne({ _id: body.bucket }, function (err, bucket) {
    if (!bucket) {
    ...
    if (bucket.user !== req.user.email) {
      return next(new errors.NotAuthorizedError());
    }

    // TODO: Remove 'body.frameSize' in next desktop release
    if (bucket.maxFrameSize && bucket.maxFrameSize !== -1 && body.frameSize && bucket.maxFrameSize < body.frameSize) {
      return next(new errors.TransferRateError('Frame is bigger than bucket limit'));
    }
...
FramesRouter.prototype._selectFarmers = function (excluded, callback) {
  ...
  async.parallel([
    (next) => Contact.find({ _id: { $in: this.CLUSTER, $nin: excluded } }).sort({ _id: -1 }).exec(next),
  ], (err, results) => {
    if (err) {
      return callback(err);
    }
