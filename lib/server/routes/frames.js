...
FramesRouter.prototype._publishContract = function (nodes, contract, audit, callback) {
  ...
  this.contracts.load(hash, (err, item) => {
    if (err) {
      ...
      const farmerContact = storj.Contact(_.sample(nodes));
      const farmerContract = storj.Contract(contract);
      ...
      this.contracts.save(item, (err) => {
        ...
        callback(null, farmerContact, farmerContract, crypto.randomBytes(20).toString('hex'));
      });
  });
};
...
FramesRouter.prototype.addShardToFrame = function (req, res, next) {
  ...
  async.series([
    ...
    const farmerUrl = `http://${address}:${port}/upload/link/${hash}`;
