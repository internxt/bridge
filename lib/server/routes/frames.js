...
function getStorageLimit(storage, user) {
  return new Promise((resolve, reject) => {
    ...
    storage.models.User.findOne({ _id: user }, function (err, _user) {
      ...
      const {maxSpaceBytes, totalUsedSpaceBytes } = _user

      resolve({ error: null, statusCode: 200, maxSpaceBytes, totalUsedSpaceBytes });
    });
...
async function userHasFreeSpaceLeft(storage, user) {
  const { maxSpaceBytes, totalUsedSpaceBytes } = await getStorageLimit(storage, user);

  let usedSpaceBytes

  if (totalUsedSpaceBytes){
    usedSpaceBytes = totalUsedSpaceBytes
  } else {
    const usage = await getStorageUsage(storage, user);
    usedSpaceBytes = (usage && usage.total) || 0;
    storage.models.User.updateOne({ _id: user }, { totalUsedSpaceBytes: usedSpaceBytes }, (err) => { if (err) logger.warn('Could not update totalUsedSpaceBytes') })
  }
