...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  if (metadata.planType === 'one_time') {

    const userNotExists = !user;

    if (userNotExists) {
      try {
        await this._createAccount(email);
      } catch (err) {
        log.error(`Error creating lifetime account ${email}. Message: ${err.message}`);

        return res.status(500).send({ info: 'Error create Account', msg: err.message });
      }
    }

    try {
      await this._updateAccountStorage(email, metadata.maxSpaceBytes);
    } catch (err) {
