...
const { getMetadata } = require('../stripeService');
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  const metadata = await getMetadata(charge).catch((err) => {
    log.error(`Error geting the metadata of a payment ${email}. Error: ${err.message}`);
  });
