...
StripeRouter.prototype._updateToLifetimeAccount = function (email, lifetime_tier) {
  const { User } = this.storage.models;

  return User.findOne({ _id: email.toLowerCase() }).then((user) => {
    if (!user) {
      throw new Error('Could not update the user plan to lifetime.');
    }
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    try {
      const plan = {
        ...
        limit: bytes.parse(lifetime_tier),
        type: 'one_time'
      };
