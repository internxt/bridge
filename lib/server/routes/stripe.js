...
StripeRouter.prototype._webhookCheckoutSessionCompleted = async function (data, res) {
  ...
  if (paymentStatus !== 'paid') {
    log.info(`Checkut processed without action, ${email} has not paid successfully`);

    return res.status(200).send({ info: `Checkut processed without action, ${email} has not paid successfully` });
  }
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  return res.status(200).send({ info: 'Endpoint available' });
