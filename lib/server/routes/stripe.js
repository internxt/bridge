...
const { trackImpact } = require('../../analytics');

const FREE_PLAN_SPACE = 2147483648;
...
StripeRouter.prototype._webhookCheckoutSessionCompleted = async function (data, res) {
  ...
  trackImpact(data.data.object.metadata).catch((err) => {
    log.error(`[Analytics] Error impact conversion: ${email}. Message: ${err.message}`);
  });
  ...
  try {
    const plan = {
      name: metadata.name,
      limit: metadata.maxSpaceBytes || FREE_PLAN_SPACE,
...
StripeRouter.prototype._webhookCustomerSubscriptionDeleted = function (data, res) {
  ...
  stripe.customers.retrieve(customer, (err, customer_obj) => {
    ...
    this._updateAccountStorage(email, FREE_PLAN_SPACE)
      .then(() => {
        log.info(`[PAYMENTS] Subscription Deleted ${email}`);

        return res.status(200).send({ info: `Subscription deleted. User ${email}` });
      })
      .catch((err) => {
...
StripeRouter.prototype._webhookTeamCheckoutSessionCompleted = async function (data, res) {
  ...
  try {
    ...
    const account = await User.findOne({ email: teamEmail });
...
StripeRouter.prototype._webhookCustomerCreated = async function (data, res) {
  const stripe = this._getStripe(!data.livemode);
  const customer = data.data.object;
  const customerId = customer.id;
  let email = customer.email;
  email = email === null ? email : email.toLowerCase();
  ...
  const customerUpdated = await stripe.customers.update(
    ...
    log.warn('[PAYMENTS] Webhook. Can`t update customer %s', message);
...
StripeRouter.prototype.parseWebhook = function (req, res, next) {
  ...
  switch (webhookObject.type) {
    ...
    case 'customer.created':
      return this._webhookCustomerCreated(webhookObject, res, next);
    default:
