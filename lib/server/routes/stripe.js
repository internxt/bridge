...
const { getSubscriptionMetadata } = require('../stripeService');
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  let metadata;
  // One Time Payments do not have invoice
  if (!invoice) {
    metadata = charge.metadata;
  } else {
    metadata = getSubscriptionMetadata(invoice).catch((err) => {
      log.error(`Error geting the metadata of a subscription ${email}. Error: ${err.message}`);
    });
  }
