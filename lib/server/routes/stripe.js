...
StripeRouter.prototype._createAccount = function (email, maxSpaceBytes, plan = 'lifetime') {
  return axios.post(`${this.config.drive.api}/api/appsumo/register`, { email: email, plan: plan, maxSpaceBytes });
};

StripeRouter.prototype._updateToLifetimeAccount = async function (email, maxSpaceBytes) {
  const User = this.storage.models.User;
  const user = await User.findOne({ _id: email.toLowerCase() });
  if (user) {
    user.maxSpaceBytes = maxSpaceBytes;
    await user.save();
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  if (metadata.member_tier === 'lifetime') {
    ...
    if (userNotExists) {
      try {
        await this._createAccount(email, metadata.maxSpaceBytes, 'lifetime');
      } catch (e) {
    ...
    try {
      await this._updateToLifetimeAccount(email, metadata.maxSpaceBytes);
    } catch (err) {
...
StripeRouter.prototype._createOrUpdateDrivePlan = function (body) {
