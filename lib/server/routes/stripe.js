...
StripeRouter.prototype._getMetadata = function (data) {
  return new Promise((resolve, reject) => {
    const charge = data.data.object.charges.data[0];
    const stripe = this._getStripe(!data.livemode);
    const invoice_id = charge.invoice;
    // Subscriptions have invoice, one time payments don't
    if (!invoice_id) {
      if (!_.isEmpty(charge.metadata)) {
        stripe.prices.retrieve(charge.metadata.price_id)
          .then((price) => resolve(price.metadata))
          .catch((err) => reject(err));
      } else {
        reject(new Error('Price metadata needed'));
      }
    } else {
      stripe.invoices.retrieve(invoice_id)
        .then((invoice) => stripe.subscriptions.retrieve(invoice.subscription))
        .then((subscription) => subscription.metadata)
        .then((metadata) => stripe.prices.retrieve(metadata.price_id))
        .then((price) => resolve(price.metadata))
        .catch((err) => reject(err));
    }
  });
...
StripeRouter.prototype._webhookPaymentIntentSucceeded = async function (data, res) {
  ...
  const metadata = await this._getMetadata(data).catch((err) => {
    log.error(err.message);

    return res.status(500).send({ info: `Metadata needed. Email: ${email}` });
  });
