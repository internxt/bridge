...
StripeRouter.prototype._webhookCheckoutSessionCompleted = async function (data, res) {
  ...
  try {
    ...
    if (err.status === 304) {
      log.error(`[PAYMENTS] Error updating user: ${email} Storage bought: ${maxSpaceBytes}. Message: ${err.message}`);

      return res.status(304).send(err);
    }
    log.error(`[PAYMENTS] Error creating user: ${email} Storage bought: ${maxSpaceBytes}. Message: ${err.message}`);
  ...
  try {
    ...
    log.error(`[PAYMENTS] Error creating drive plan for user ${email}: ${err.message}`);
  }
...
StripeRouter.prototype._webhookCustomerSubscriptionDeleted = function (data, res) {
  ...
  stripe.customers.retrieve(customer, (err, customer_obj) => {
    ...
    User.findOne({ _id: email }, (err, user) => {
      if (err || !user) {
        log.error('[PAYMENTS] Webhook error, user %s not found on bridge database', email);

        return res.status(200).send({ error: 'Unkown customer on bridge' });
      }

      this._updateAccountStorage(email, 0).catch(() => {
        log.error('[PAYMENTS] Webhook error, cannot update %s on bridge');
