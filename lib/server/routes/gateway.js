...
class GatewayRouter extends Router {
  ...
  async planUpgrade(req, res, next) {
    ...
    try {
      user = await this.storage.models.User.findOne({ email });
      ...
      logger.error(`[GATEWAY/PLAN_UPGRADE] - Error upgrading plan for user ${user.uuid}: ${err.message}. Payload: ${JSON.stringify(req.body)}`);
  ...
  async getUuid(req, res, next) {
    try {
      const user = await this.storage.models.User.findOne({ email: req.body.email });
      if (!user) {
        return next(errors.BadRequestError('User not found'));
      }
  ...
  async incrementStorage(req, res, next) {
    ...
    try {
      const user = await this.storage.models.User.findOne({ email });

      if (!user) {
        logger.error(`User not found (when incrementing storage by email): ${email}`);
  ...
  async incrementStorageByUUID(req, res, next) {
    ...
    try {
      ...
      if (!user) {
        logger.error(`User not found (when incrementing storage by uuid): ${uuid}`);
