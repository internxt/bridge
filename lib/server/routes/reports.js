...
class ReportsRouter extends Router {
  ...
  constructor(options) {
    ...
    this.CLUSTER = Object.values(this.config.application.CLUSTER || []);
  }
  ...
  async createExchangeReportAuth(req, res, next) {
    const { ExchangeReport } = this.storage.models;

    authFarmer(req, res, (err) => {
      req.signed = true;
      if (err) {
        req.signed = false;
      }
      ...
      const farmerId = req.headers['x-node-id'];
      if (!err) {
        if (farmerId !== exchangeReport.reporterId) {
          log.error('Exchange report error: Signed farmer %s is reporting about %s', farmerId, exchangeReport.farmerId)
          req.signed = false;
        }
      }

      return this.createExchangeReport(req, res, next);
    })
  }
  ...
  async createExchangeReport(req, res, next) {
    ...
    if (req.signed) {
      this._handleSignedExchangeReport(exchangeReport);
    } else {
  ...
  _handleSignedExchangeReport(report) {
    return this._handleExchangeReport(report);
  }
        ...
        this._triggerMirrorEstablish(dataHash).then(() => {
          ...
          switch (err.message) {
            case 'Shard already in cluster':
              break;
            case '':
              log.error('Error establishing new mirror without error message');
              break;
            default:
  ...
  _getMirrors(shard, cb) {
    ...
    return Mirror.find({ shardHash: shard.hash, contact: { $in: this.CLUSTER } })
      .populate('contact')
      .exec((err, mirrors) => cb(err, shard, mirrors));
  }

  _isMirroringNeeded(shard, mirrors, cb) {
    const isInCluster = mirrors.filter((v) => this.CLUSTER.indexOf(v) > -1 && v.isEstablished === true);

    if (isInCluster.length > 0) {
  ...
  _triggerMirrorEstablish(hash) {
    return async.waterfall([
      ...
      this._isMirroringNeeded.bind(this),
      this._getMirrorCandidate.bind(this),
  ...
  _definitions() {
    return [
      ['POST', '/reports/exchanges', this.getLimiter(limiter(1000)), rawBody, this.createExchangeReportAuth.bind(this)],
