...
const { BucketsUsecase, BucketEntryNotFoundError, BucketEntryFrameNotFoundError, BucketNotFoundError, BucketForbiddenError, MissingUploadsError, MaxSpaceUsedError, InvalidUploadIndexes, InvalidMultiPartValue } = require('../../core/buckets/usecase');
...
BucketsRouter.prototype.startUpload = async function (req, res, next) {
  ...
  const multiparts = +(req.query.multiparts || 1);
  const totalSize = uploads.reduce((acc, { size }) => acc + size, 0);
  const MB500 = 500 * 1024 * 1024;
  if (totalSize < MB500 && multiparts > 1) {
    return next(new errors.BadRequestError('Multipart is not supported for files smaller than 500MB'));
  }
  ...
  try {
    ...
    if (err instanceof InvalidMultiPartValue) {
