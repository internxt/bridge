...
const { removeFileAndEnqueueDeletionTask } = require('../services/files');
...
BucketsRouter.prototype.removeFile = async function (req, res, next) {
  const { id: bucketId, file: idFile } = req.params;
  const { _id: userEmail } = req.user;
  try {
    await removeFileAndEnqueueDeletionTask(this.storage, this.networkQueue, {
