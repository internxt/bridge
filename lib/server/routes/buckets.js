    ...
    Frame.findOne({
      ...
      BucketEntry.create({
        ...
        if (req.user.totalUsedSpaceBytes)
          User.updateOne({ _id: req.user._id }, {$inc: { totalUsedSpaceBytes: size}}, (err) => { if (err) logger.warn('Could not update totalUsedSpaceBytes') })
...
BucketsRouter.prototype.removeFile = async function (req, res, next) {
  ...
  if (req.user.totalUsedSpaceBytes)
    User.updateOne({ _id: req.user._id }, {$inc: { totalUsedSpaceBytes: -frame.size}}, (err) => { if (err) log.warn('Could not update totalUsedSpaceBytes') })
