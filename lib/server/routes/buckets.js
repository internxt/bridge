...
BucketsRouter.prototype.startUpload = async function (req, res, next) {
  ...
  try {
    ...
    const uploadPromises = uploads.map(async upload => {
      ...
      const [contact, savedUpload] = await Promise.all([
...
BucketsRouter.prototype.finishUpload = async function (req, res, next) {
  ...
  try {
    ...
    const shardPromises = shards.map(async shard => {
      ...
      const [savedShard, ...savedMirrors] = await Promise.all([
    ...
    res.status(200).send({ ...savedBucketEntryObject, shards: savedShards });
