  ...
  Bucket.findOne({
    ...
    async.eachLimit(
      ...
      (shard, cb) => {
        ...
        if (!validateUUID(uuid, 4)) {
          return cb(new errors.BadRequestError('Invalid UUID'));
        }
          ...
          Shard.create(shardToCreate, (err, savedShard) => {
            ...
            async.eachLimit(
              ...
              (contract, cb) => {
                ...
                mirrorToCreate.save((err, savedMirror) => {
                  if (err) {
                    return cb(err);
                  }
                  cb();
      ...
      (err) => {
        ...
        BucketEntry.create(bucketEntryToCreate, (err, savedBucketEntry) => {
          if (err) {
            return next(new errors.InternalError(err.message));
          }
