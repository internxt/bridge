...
BucketsRouter.prototype.getDownloadLinks = function (req, res, next) {
  ...
  const { BucketEntry, Shard, Mirror } = this.storage.models;
    ...
    async.mapLimit(
      ...
      (shardId, nextShardId) => {
        ...
        Shard.findOne({
          ...
          if (err) {
            return nextShardId(new errors.InternalError(err.message));
          }
          ...
          Mirror.findOne({
            ...
            // self._getRetrievalToken(mirror, {}, (err, result) => {
            //   if (err) {
            //     return nextShardId(new errors.InternalError(err.message));
            //   }

            //   const { hash, farmer } = result;
            ...
            //   const { address, port } = farmer;
            //   const farmerUrl = `http://${address}:${port}/download/link/${hash}`;
            //   const headers = { headers: { 'X-TOKEN': result.token } };

            //   return axios.get(farmerUrl, headers).then(res => ({ ...result, url: res.data.result }));
            // });
          });
        });
      }, (err, shards) => {
    ...
    async.eachLimit(
      ...
      (shard, cb) => {
        ...
        Upload.findOne({
          ...
          if (!upload) {
            return cb(new errors.NotFoundError('Upload not found'));
          }

          const uploadObject = upload.toObject();
          const { _id, contracts, ...uploadWithoutIdAndContracts } = uploadObject;

          const shardToCreate = {
            ...uploadWithoutIdAndContracts,
            hash,
          ...
          Shard.create(shardToCreate, (err, savedShard) => {
            if (err) {
              return cb(err);
            }

            createdShardIDs.push(savedShard._id);
