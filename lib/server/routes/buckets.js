    ...
    User.findOne({
      ...
      BucketEntry.findOne({
        ...
        self._getPointersFromEntry(entry, {
          ...
          // This endpoint returns an empty array when no more mirrors are available
          if (result.length === 0) {
          ...
          async.eachLimit(result, 5, (pointer, nextPointer) => {
            const { hash, farmer } = pointer;
            ...
            axios.get(farmerUrl, { auth: { username, password } }).then((farmerRes) => {
              pointers.push({ ...pointer, url: farmerRes.data.result });
              nextPointer();
            }).catch((err) => {
              nextPointer(err);
            });
