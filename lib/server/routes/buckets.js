    ...
    async.mapLimit(
      ...
      (err, uploads) => {
        ...
        bucketEntryToCreate.save((err, savedBucketEntry) => {
          ...
          const bucketEntryShardsToCreate = createdShards.map((shard, index) =>
            new BucketEntryShard({
          ...
          async.parallel(
            bucketEntryShardsToCreate.map((bucketEntryShard) => bucketEntryShard.save.bind(bucketEntryShard)),
            (err, results) => {
              if (err) {
                return next(err);
              }

              res.status(200).send({ ...savedBucketEntryObject, shards: createdShards });
