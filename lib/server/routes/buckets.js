...
const { BucketsUsecase, BucketEntryNotFoundError, BucketEntryFrameNotFoundError, BucketNotFoundError, BucketForbiddenError, MissingUploadsError, MaxSpaceUsedError } = require('../../core/buckets/usecase');
...
/**
 * Creates a bucket entry from the given frame object (checks shard existence from farmer first)
 * @param {http.IncomingMessage} req
 * @param {http.ServerResponse} res
 * @param {Function} next
 */
// eslint-disable-next-line complexity
BucketsRouter.prototype.ensureCreateEntryFromFrame = async function (req, res, next) {
  const { Frame, Bucket, BucketEntry, Pointer, Mirror } = this.storage.models;
  ...
  try {
    ...
    if (!frame.shards || !frame.shards.length === 0) {
      return next(new errors.ConflictError());
    }

    const pointer = await Pointer.findOne({ _id: frame.shards[0] });

    if (!pointer) {
      return next(new errors.ConflictError('Pointer not found'));
    }

    const mirror = await Mirror.findOne({ shardHash: pointer.hash }).populate('contact').exec();

    if (!mirror || !mirror.contact) {
      return next(new errors.ConflictError('Mirror not found'));
    }

    const { address, port } = mirror.contact;

    // if shard not exists, it will throw
    await axios.get(`http://${address}:${port}/shards/${mirror.shardHash}/exists`);
...
BucketsRouter.prototype.getFile = function (req, res, next) {
  const self = this;
  const { Bucket, BucketEntry, User, BucketEntryShard, Shard, Mirror, Contact } = this.storage.models;
    ...
    User.findOne({
      ...
      BucketEntry.findOne({
        ...
        if (entry.version && entry.version === 2) {
          BucketEntryShard.find({
            bucketEntry: entry._id
          }).sort({
            'index': 1
          }).limit(Math.min(req.query.limit, 6)).exec((err, bucketEntryShards) => {
            if (err) {
              return next(err);
            }

            if (bucketEntryShards.length === 0) {
              return next(errors.NotFoundError('Bucket entry shard not found for bucket entry ' + entry._id));
            }

            const shardIds = bucketEntryShards.map(b => b.shard);

            Shard.find({ _id: { $in: shardIds } }, (err, shards) => {
              if (err) {
                return next(err);
              }

              if (shards.length === 0) {
                return next(errors.NotFoundError('Shards not found for shard ids' + shardIds.toString()));
              }

              const shardHashes = shards.map(s => s.hash);
              const mirrors = [];

              async.eachLimit(shardHashes, 1, (shardHash, done) => {
                Mirror.findOne({ shardHash }, (err, mirror) => {
                  if (err) {
                    return done(err);
                  }
                ...
                const shardsWithMirrors = [];

                async.eachLimit(mirrors, 1, (mirror, nextMirror) => {
                  const shard = shards.find(s => s.hash === mirror.shardHash);
                  const bucketEntryShard = bucketEntryShards.find(be => {
                    return be.shard.toString() === shard._id.toString();
                  });

                  Contact.findOne({ _id: mirror.contact }, (err, contact) => {
                    if (err) {
                      return nextMirror(err);
                    }

                    if (!contact) {
                      return nextMirror(errors.NotFoundError('Contact not found for mirror ' + mirror._id));
                    }

                    const { address, port } = contact;
                    const farmerUrl = `http://${address}:${port}/v2/download/link/${shard.uuid}`;

                    axios.get(farmerUrl).then(res => res.data.result).then((url) => {
                      shardsWithMirrors.push({
...
BucketsRouter.prototype.getFiles = async function (req, res, next) {
  try {
    ...
    const hashes = list.map(entry => entry.frame.shards[0].hash);
    ...
    const response = fileIdList.map((fileId) => {
      const i = list.findIndex((entry) => entry._id.toString() === fileId);

      return { fileId, link: links[i], index: indexes[i] };
    });
...
BucketsRouter.prototype.getFileInfo = function (req, res, next) {
  this._getBucketUnregistered(req, res, (err, bucket) => {
    ...
    const supportsV2 = req.headers['x-api-version'] && req.headers['x-api-version'] === '2';

    this.usecase.getFileInfo(bucket._id, req.params.file, supportsV2).then((fileInfo) => {
...
BucketsRouter.prototype.startUpload = async function (req, res, next) {
  ...
  const uploadIndexesWithoutDuplicates = new Set(uploads.map((upload) => upload.index));

  if (uploadIndexesWithoutDuplicates.size < uploads.length) {
    return next(new errors.BadRequestError('Invalid uploads indexes'));
  }

  const bucketEntrySize = uploads.reduce((acumm, upload) => upload.size + acumm, 0);

  try {
    if (req.user.migrated) {
      if (user.maxSpaceBytes < user.totalUsedSpaceBytes + bucketEntrySize) {
        return next(new errors.TransferRateError('Max space used'));
      }
    } else {
      const usedSpaceBytes = await this.usecase.getUserUsage(req.user._id);
    ...
    const uploadPromises = uploads.map(async upload => {
      ...
      const uploadToBeCreated = new Upload({
        uuid,
        index,
        contracts,
        data_size: size
      ...
      const [contact] = await Promise.all([
        Contact.findOne({ _id: nodeID }),
        uploadToBeCreated.save()
      ]);
      ...
      return { index, uuid, url: objectStorageUrl };
    });

    const uploadsResult = await Promise.all(uploadPromises);

    res.status(200).send({ uploads: uploadsResult });
  } catch (err) {
    if (err.statusCode) {
      return next(err);
    }
