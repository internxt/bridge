  ...
  try {
    ...
    Pointer.find({ _id: { $in: frame.shards } }, (err, pointers) => {
      ...
      async.eachSeries(pointers, (pointer, nextPointer) => {
        pointer.remove((err) => {
          if (err) {
            log.error('removeFile/Error deleting pointer %s: %s', pointer._id, err.message);
          } else {
            this.networkQueue.enqueueMessage({
              ...
              if (err) {
                log.error('removeFile/Error enqueueing shard %s deletion task: %s', pointer.shard, err.message); 
              } else {
                log.info('removeFile/Job sent to delete shard: %s', pointer.hash);
              }
            });
          }
          nextPointer();
