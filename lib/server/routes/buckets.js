...
BucketsRouter.prototype.getFiles = async function (req, res, next) {
  try{
    ...
    const response = fileIdList.map((fileId,i) => ({fileId, link: links[i], index: indexes[i]}));
...
BucketsRouter.prototype.removeFile = async function (req, res, next) {
  const { Bucket, BucketEntry, Frame, Pointer } = this.storage.models;
  ...
  try {
    ...
    // TODO: Is this required considering frame population?
    const frame = await Frame.findOne({ _id: bucketEntry.frame.id });

    if (!frame) {
      log.error('removeFile/Frame %s not found for file %s', bucketEntry.frame.id, bucketEntry._id);
    ...
    Pointer.find({ _id: { $in: frame.shards } }, (err, pointers) => {
      if (err) {
        return log.error('removeFile/Error finding pointers for frame %s: %s', frame._id, err.message);
      }

      this.deletePointers(pointers).catch((err) => {
        log.error('removeFile/deletePointers: Error deleting pointers for frame %s: %s', frame._id, err.message);
      });
    });
...
BucketsRouter.prototype.finishUpload = async function (req, res, next) {
  const { Bucket, BucketEntry, Upload, Shard, Mirror, BucketEntryShard } = this.storage.models;
  ...
  try {
    ...
    const shardPromises = shards.map(async shard => {
      ...
      if (!upload) {
        throw new errors.NotFoundError('Upload not found');
      }

      const uploadObject = upload.toObject();
      const { index, data_size, contracts } = uploadObject;
      ...
      const mirrorsToBeCreated = contracts.map(contract => {
        ...
        return mirrorToCreate.save();
      });

      const [savedShard] = await Promise.all([
        shardToCreate.save(), ...mirrorsToBeCreated
      ]);

      uploadsToBeRemoved.push(upload);
    ...
    const [savedBucketEntry, ...savedShards] = await Promise.all([
      bucketEntryToCreate.save(),
      ...shardPromises
    ]);

    const savedBucketEntryId = savedBucketEntry._id;

    const bucketEntryShardsToCreate = savedShards.map((shard, index) => {
      const bucketEntryShardToCreate = new BucketEntryShard({
        bucketEntry: savedBucketEntryId,
        shard: shard._id,
        index
      });

      return bucketEntryShardToCreate.save();
    });

    await Promise.all(bucketEntryShardsToCreate);

    const savedBucketEntryObject = savedBucketEntry.toObject();

    res.status(200).send({ ...savedBucketEntryObject, id: savedBucketEntryId });

    const uploadPromises = uploadsToBeRemoved.map(upload => upload.remove());
