...
BucketsRouter.prototype.finishUpload = async function (req, res, next) {
  ...
  for (const { uuid, hash, UploadId } of shards) {
    ...
    if (UploadId !== undefined && (typeof UploadId !== 'string' || UploadId.length === 0)) {
      return next(new errors.BadRequestError('Invalid UploadId'));
    }
  }
