...
const DELETING_FILE_MESSAGE = require('../queues/message_types').DELETING_FILE_MESSAGE;
...
function BucketsRouter(options) {
  ...
  this.network_queue = options.network_queue;
}
          ...
          Pointer.find({ _id: { $in: frame.shards } }, (err, pointers) => {
            ...
            async.eachSeries(pointers, (pointer, nextPointer) => {
              ...
              pointer.remove((err) => {
                ...
                if(!err){
                  this.network_queue.enqueueTask({
                    type: DELETING_FILE_MESSAGE, 
                    payload: { hash: pointer.hash }
                  }, (err) => {
                    if(err){
                      log.error('Error adding deleting task to the queue of shard: %s', pointer.shard);
                      return;
                    }
                    log.info('Job sent to delete shard: %s', pointer.hash);
                  })
                }
                nextPointer();
              }, () => {
