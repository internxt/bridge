...
const publicBucket = require('../middleware/public-bucket');
...
const { formatShardHash } = require('../../core/shards/utils');
...
function BucketsRouter(options) {
  ...
  this._verify = authenticate({ User: this.storage.models.User, PublicKey: this.storage.models.PublicKey, UserNonce: this.storage.models.UserNonce });
  this._isPublic = publicBucket(this.storage);
  this._usetoken = tokenauth(this.storage);
  this.getLimiter = rateLimiter(options.redis);
...
BucketsRouter.prototype.getBuckets = async function (req, res, next) {
  ...
  try {
    ...
    log.error('Error getting buckets for user %s: %s', userId, err.message);
...
BucketsRouter.prototype.createBucket = async function (req, res, next) {
  ...
  const pubkeys = Array.isArray(req.body.pubkeys) ? req.body.pubkeys : [];
  if (req.pubkey && !pubkeys.includes(req.pubkey._id)) {
    pubkeys.push(req.pubkey._id);
  }
  ...
  try {
    const createdBucket = await this.usecase.create(req.user, req.body);
 ...
 * @param {Function} next
 */
BucketsRouter.prototype.destroyBucketById = async function (req, res, next) {
  try {
    const userId = req.user.uuid;
    ...
    await this.usecase.deleteBucketByIdAndUser(bucketId, userId);
...
BucketsRouter.prototype._getRetrievalToken = function (sPointer, opts, done) {
  ...
  this.contracts.load(sPointer.hash, async function (err, item) {
    ...
    try {
      const contacts = await self.storage.models.Contact.find({
        _id: { $in: farmers }
      });
...
BucketsRouter.prototype.getFileInfo = async function (req, res, next) {
  const getBucketUnregisteredAsync = (req, res) => {
  ...
  try {
    const bucket = await getBucketUnregisteredAsync(req, res);
    ...
    await this.fileStateUsecase.updateOrSetLastAccessDate(req.params.file)
      .then(() => {
        log.info('[UPDATE_LAST_ACCESS_DATE] getFileInfo: Updated last access date', { userId: req.user.uuid, fileId: req.params.file });
      })
      .catch(error =>
        log.error('[UPDATE_LAST_ACCESS_DATE] getFileInfo: Error updating last access date', { userId: req.user.uuid, fileId: req.params.file, error: error.message })
      );
    ...
    if (err instanceof errors.NotFoundError || err instanceof errors.NotAuthorizedError) {
      return next(err);
    }
    ...
    log.error('getFileInfo: Error for file %s: %s. %s, details: %s', req.params.file, err.message, err.stack, JSON.stringify(errorDetails));
...
BucketsRouter.prototype.startUpload = async function (req, res, next) {
  ...
  try {
    ...
    log.error('startUpload: Error for bucket %s: for user: %s %s. %s, details: %s', bucketId, req.user.uuid, err.message, err.stack, JSON.stringify(errorDetails));
...
BucketsRouter.prototype.finishUpload = async function (req, res, next) {
  ...
  try {
    ...
    await this.fileStateUsecase.updateOrSetLastAccessDate(bucketEntry.id)
      .then(() => {
        log.info('[UPDATE_LAST_ACCESS_DATE] finishUpload: Updated last access date', { userId: req.user.uuid, fileId: bucketEntry.id });
      })
      .catch(error =>
        log.error('[UPDATE_LAST_ACCESS_DATE] finishUpload: Error inserting last access date', { userId: req.user.uuid, fileId: bucketEntry.id, error: error.message })
      );
...
BucketsRouter.prototype.getDownloadLinks = async function (req, res, next) {
  ...
  const shardsWithoutUuid = response.map((res) => formatShardHash(res));
