...
function identifyReferral(userId, traits) {
  ...
  try {
    ...
    log.error(`[Analytics] Identify Referrals. Error: ${err.message}`);
  }
}

async function trackImpact(metadata) {
  if (metadata.source !== 'website' || !metadata.impactId) {
    return;
  }
  const segmentKey = process.env.NODE_ENV === 'development' ? config.api_keys.segment_test : config.api_keys.segment;
  const analytics = new Analytics(segmentKey, { flushAt: 1 });
  ...
  if (metadata.impact_id) {
    properties.impact_id = metadata.impactId;
    properties.affiliate = 'impact';
  }
